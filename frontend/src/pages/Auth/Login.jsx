...
const Login = () => {
  ...
  return (
    /* min-h-[100dvh]: Essential for mobile browsers to avoid the URL bar push-down.
       px-4: Prevents the card from touching the edges on small phones.
    */
    <div className="flex flex-col items-center justify-center min-h-[100dvh] w-full px-4 py-12 relative overflow-hidden bg-black text-white">
      {/* Back Button: Moved to a safe inset for mobile touch targets */}
      <button
        onClick={() => navigate("/")}
      ...
      {/* Login Card: 
          - max-w-[90vw] on tiny screens
          - p-6 on mobile, p-8 on tablet, p-12 on desktop
      */}
      <div className="bg-gray-900/80 p-6 sm:p-8 md:p-12 rounded-[2rem] sm:rounded-3xl shadow-2xl border border-gray-800 w-full max-w-[340px] sm:max-w-sm md:max-w-md text-center relative z-10 backdrop-blur-md">
        ...
        {/* Title & Subtitle: Scale text for smaller viewports */}
        ...
        {/* Google Login Button: Optimized height and font for mobile thumbs */}
        <button
          onClick={() =>
        ...
        {/* Footer: Tracking adjusted for better mobile legibility */}
      ...
      {/* Background Decorative Element (Optional but adds depth for tablets) */}
